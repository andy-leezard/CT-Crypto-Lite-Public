...
const EditUsername:React.FC<Props> = ({route, navigation}) => {
    ...
    const [msg_username_error, setmsg_username_error] = useState<string>(I18n.t('er_format'));
    ...
    return (
        <View style={{flex:1,paddingTop:15}}>
            <View>
                <Text style={{fontSize:17,fontWeight:"600",letterSpacing:0.5,color:textColor(globalContext.state.env.darkmode!),marginLeft:15,marginBottom:10}}>{I18n.t('new_username')}</Text>
                ...
                <TextInput style={[styles.input,{backgroundColor:containerColor(globalContext.state.env.darkmode!),borderColor:containerRadiusColor(globalContext.state.env.darkmode!),color:textColor(globalContext.state.env.darkmode!)}]} value={newusername} onChangeText={setnewusername} maxLength = {20}/>
