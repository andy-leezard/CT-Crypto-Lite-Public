...
import { textColor, dynamicColor_marketChange } from "./StyleLib";
import { fastRound, addZeroes } from "./FuncLib";
import stablecoins from '../stablecoins.json';

export const min_threshold = (i) => {return i<0.000001 ? 0:i;}

export const min_threshold_sell = (i) => {return i<0.00000001 ? 0:i;}
...
export const parse_global_data = (_res) => {
    ...
    let obj = {
        "data":_res,
        "color":dynamicColor_marketChange(percentage),"market":asString(fastRound(percentage)),
        "stabd":(addZeroes(fastRound(sum_stablecoins))+"%"),
        "_no1":tops[0], "_no2":tops[1], "_no3":tops[2]
    };
    return obj
}